...
# Variables for this job
  ...
  KEY4=FB3K     # NOTE: Get the key with register_key.pl below !
...
# the location of the pandagma job.
  cp -r /project/legume_project/$USER/26/$GENUS/out_pandagma .
...
# Move files in collection directory into place in the ANNEX data store
  mkdir -p $ANNEX/$GENUS/GENUS/pangenes/$GENUS.$PANNUM.$KEY4
