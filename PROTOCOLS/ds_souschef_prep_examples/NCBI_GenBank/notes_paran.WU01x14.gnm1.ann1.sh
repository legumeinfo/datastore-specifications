...
# Data from https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_002914805.1/
#RefSeq assembly  GCA_002914805.1, assembly PanWU01x14_asm01
...
# Variables for this job
  ...
  GNM=gnm1
  ANN=ann1  
  GENOME=GCA_002914805.1_PanWU01x14_asm01_genomic.fna
...
# Get the genome assembly and annotations
  curl -O https://api.ncbi.nlm.nih.gov/datasets/v2/genome/accession/GCA_002914805.1/download?include_annotation_type=GENOME_FASTA&include_annotation_type=GENOME_GFF&include_annotation_type=RNA_FASTA&include_annotation_type=CDS_FASTA&include_annotation_type=PROT_FASTA&include_annotation_type=SEQUENCE_REPORT&hydrated=FULLY_HYDRATED
...
#   >Chr1 GWHCAYC00000001 Complete=T Circular=F OriSeqID=Gm01 Len=59641292
  ...
  cat $FROM/$GENOME |
	  perl -pe 's/^>(\S+)\s+.+PanWU01x14_asm01_(scf\d+),.+/>$2 $1/' > $TO/$ACCN.modID.genome.fasta 
    
# Also in the molecule IDs in the gff. 
  grep ">"  $FROM/$GENOME |
    perl -pe 's/^>(\S+)\s+.+PanWU01x14_asm01_(scf\d+),.+/$1 $2/' > $TO/$ACCN.initial_seqid_map.tsv
