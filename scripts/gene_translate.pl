...
my $usage = <<EOS;
  ...
  In the default mode, the FROM list is prefixed with the form gensp.strain.gnm#.ann# :
...
while (<>){
  ...
  $from_gn =~ s/(.+)\.\d{1,2}$/$1/;   # like Gene1234.1
  $from_gn =~ s/(.+)\.\D\d{1,2}$/$1/; # like Gene1234.m1

  # For case-insensitive matching, lowercase IDs in the query list
  my $from_gn_lc = lc($from_gn);

  my $pan;
  if (defined($gn_to_pan_hsh{$from_gn_lc})) {
    $pan = $gn_to_pan_hsh{$from_gn_lc};
    ...
    warn "WARNING - query gene not found in the pangene table: $from_gn_lc";
  }
}
...
if ($json){
  ...
  foreach my $from_gn (sort keys %gene_set_hsh) {
  #while ( my($from_gn, $pan_and_to_gns) = each(%gene_set_hsh)) {
    my $pan_and_to_gns = $gene_set_hsh{$from_gn};
...
2025-09-26 Add warning for query genes not found in the lookup table. Print result sorted by from_gn
